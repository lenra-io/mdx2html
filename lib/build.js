...
export function bulkBuild(fileMaps, out) {
    ...
    let transformedFileMap = fileMaps.map(e => {
        ...
        let basename = path.basename(e.source).replace(/\.[^/.]+$/, "")
        return {
            ...e,
            compiled: path.resolve(buildOutdir, basename + ".js")
        }
    })
...
async function saveHtml(fileMaps) {
    let promises =
        fileMaps.map(async (file) => {
            ...
            var json = mod.json || { "title": "None" }
            var html = toHtml(mod.default, file.props)
